...
class _EndorsementsState extends State<Endorsements> {
  ...
  Future<void> downloadAllFiles() async {
    //final url = 'http://14.99.138.131:9981/clientlist/endorsement/downloadAll';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final filesList = json.decode(response.body) as List<dynamic>;

        await Future.forEach(filesList, (fileDetails) async {
          final fileId = fileDetails[
              'id']; // Adjust this based on the actual response structure
          final fileUrl =
              'http://14.99.138.131:9981/clientlist/endorsement/downloadFileByEndorsementId?endorsementId=$fileId';
          ...
          if (fileResponse.statusCode == 200) {
            ...
            // Choose the appropriate directory (use appDocumentsDirectory or downloadsDirectory)
            final fileDirectory = appDocumentsDirectory.path;
            // final fileDirectory = downloadsDirectory.path;

            final fileName =
                'downloaded_file_$fileId.extension'; // Adjust the file name based on your requirements
            ...
            print('File $fileId downloaded successfully at $filePath');
          } else {
            print(
                'Failed to download file $fileId. Status code: ${fileResponse.statusCode}');
          }
        });
        ...
        print(
            'Failed to fetch files list. Status code: ${response.statusCode}');
      }
    } catch (e) {
