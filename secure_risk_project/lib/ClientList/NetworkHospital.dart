...
class _NetworkHospitalFormState extends State<NetworkHospitalForm> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data != null && data.isNotEmpty) {
          final List<NetworkHospital> clientsData = data
              .map((item) => NetworkHospital(
                    noworkHospitalId: item['clientDetailsId'] ?? '',
                    hospitalName: item['clientDetailsName'] ?? '',
                    address: item['fileName'] ?? '',
                    pincode: item['clientDetailsId'] ?? '',
                  ))
              .toList();
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (responseData.containsKey('clientDetailsId')) {
          ...
          if (clientIds != null) {
            if (clientIds != null) {
              var newClient = NetworkHospital(
                noworkHospitalId: clientIds,
              ...
              children: [
                ...
                Column(
                  children: [
                    ...
                    Container(
                      // height: 50,
                      height: _nameHasError == null || _nameHasError!.isEmpty
                          ? 50.0
                          : 65.0,
                    ...
                    ElevatedButton(
                      ...
                      //           SnackBar(
                      //             content:
                      //                 Text('File uploaded: $pickedFileName'),
                      //           ),
                      //         );
                      //       }
    ...
    try {
      ...
      if (clientId != null && clientId.isNotEmpty) {
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          ...
          var updateClient = NetworkHospital(
            noworkHospitalId: int.parse(clientId),
            hospitalName: nameController.text,
            address: updateFileName,
            pincode: int.parse(clientId),
          );
  ...
  downloadAllClientDetails(String fileName) async {
    String urlString =
        "${ApiServices.baseUrl}${ApiServices.clientDetailsDownloadAll_endpoint}";
                          ...
                          rows: [
                            ..._filteredData
                                .map((client) => DataRow(cells: [
                                      ...
                                      DataCell(Text("Yashodha Hospital"
                                          //  client.hospitalName ?? "N/A"
                                          )),
                                      DataCell(Text("Hyderabad"
                                          // client.address ?? "N/A"
                                          )),
                                      DataCell(Text("500055"
