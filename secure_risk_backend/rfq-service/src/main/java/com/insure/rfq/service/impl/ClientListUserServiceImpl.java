...
public class ClientListUserServiceImpl implements ClientListUserService {
	...
	public ClientListUserServiceImpl(ClientListUserRepository clientListUserRepository, ClientListRepository clientListRepository, ModelMapper modelMapper, DesignationRepository designationRepository) {
...
public String deleteClientUserById(Long clientUserId) {
    Optional<ClientListUser> optionalClientListUser = clientListUserRepository.findById(clientUserId);
    if (optionalClientListUser.isPresent()) {
        ClientListUser clientListUser = optionalClientListUser.get();
        clientListUser.setStatus("INACTIVE");
        clientListUserRepository.save(clientListUser);
        return "User Deleted Successfully";
    } else {
        throw new InvalidClientListUser("User is Not Present");
    }
}
