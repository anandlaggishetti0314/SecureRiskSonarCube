...
public class ClientDetailsClaimsMisImpl implements ClientDetailsClaimsMisService {
    ...
    public ClientDetailsClaimsMisImpl(ClaimsMisRepository claimsMisRepo, TpaRepository tpaRepo, ClientDetailsClaimsMisRepository clientDetailsClaimsMisRepository, CoverageFileUploadRepository fileUploadRepo,@Value("${file.path.coverageMain}")String mainpath, ProductRepository productRepository, ClientListRepository clientListRepository, ExcelUtils excelUtils, ClientDetailsClaimsMisNewRepo clientDetailsClaimsMisNewRepo) {
        ...
        this.clientDetailsClaimsMisNewRepo = clientDetailsClaimsMisNewRepo;
    }
    ...
    public byte[] getClaimsForEmployeeInExcelFormat(Long clientlistId, Long productId, String employeeId) {
        ...
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream()) {
            ...
            // Create headers
            Row headerRow = sheet.createRow(0);
            String[] headers = {"POLICY NUMBER", "CLAIMS NUMBER", "EMPLOYEE ID", "EMPLOYEE NAME", "RELEATIONSHIP", "GENDER", "AGE",
                    "PATIENT NAME", "SUM INSURED", "CLAIMED AMOUNT", "PAID AMOUNT", "OUTSTANDING AMOUNT", " CLAIMS STATUS", " DATE OF CLAIM",
                    "CLAIM TYPE", "NETWORK TYPE", "HOSPITAL NAME", "ADMISSION DATE", " DIESEASE", "DATE OF DISCHARGE", " MEMBER CODE", "POLICY START DATE",
                    " POLICY END DATE", " HOSPITAL STATE", "HOSPITAL CITY "};
            for (int i = 0; i < headers.length; i++) {
...
//  OutStanding Under process and Reimbursement
    ...
    public void validateBasedOnSheet(Sheet sheet, String tpaName,
        ...
        Tpa tpadata = tpaRepo.findByTpaName(tpaName);
        List<ClaimsTPAHeaders> tpaHeadersBasedOnTPA = tpadata.getTpaHeaders();
    ...
    public String uploadFileCoverage(ClientDetailsClaimsMisUploadDto coverageUploadDto, Long clientlistId,
                                     Long productId) {
        ...
        if (coverageUploadDto.getFileType().equals("ClaimsMis")) {
            ...
            for (ClientDetailsClaimsMis clientDetailsClaimsMis1 : claimsMisData) {
                ...
                clientDetailsClaimsMis1.setCreatedDate(new Date());

            }

            log.error("	@Slf4j :{}", claimsMisData);
            List<ClientDetailsClaimsMis> saveAll = clientDetailsClaimsMisRepository.saveAll(claimsMisData);
            List<ClientDetailsClaimsMis> findByRfqId = clientDetailsClaimsMisRepository
                    .findByRfqId(saveAll.stream().map(ClientDetailsClaimsMis::getRfqId).distinct().toString());
	...
	@Override
	public ClientListClaimsTrackerDto getClaimDetailsForEmployee(Long clientId, Long productId, String employeeId) {
		 List<ClientListClaimsTrackerDto> resultList = clientDetailsClaimsMisRepository.findAll().stream()
		    ...
		    if (!resultList.isEmpty()) {
		        return resultList.get(0); // Return the first (and only) element
		    } else {
