...
public class CorporateDetailsServiceImpl implements CorporateDetailsService {
	...
	public CorporateDetailsServiceImpl(@Value("${spring.mail.host}")String host, @Value("${spring.mail.username}")String username, @Value("${spring.mail.password}")String password, @Value("${spring.mail.port}")int port,@Value("${spring.mail.properties.mail.smtp.auth}") String smtpAuth, @Value("${spring.mail.properties.mail.smtp.starttls.enable}")String starttls, CorporateDetailsRepository corporateDetailsRepo, @Value("${file.path.corporatepath}")String emailUrl, ProductCategoryRepository prodCategoryRepo, ProductRepository prodRepo, CoverageDetailsRepository coverageDetailsRepo, PolicyTermsRepository policyTermsRepo, ExpiryPolicyDetailsRepository expiryPolicyDetailsRepository, LocationRepository locationRepository, ClientListRepository clientListRepo, EmployeeRepository employeeRepository, IrdaPdfGenerator irda, ClaimsDetailsRepository claimsDetailsRepository) {
		this.host = host;
		...
		this.corporateDetailsRepo = corporateDetailsRepo;
		this.emailUrl = emailUrl;
		this.prodCategoryRepo = prodCategoryRepo;
		this.prodRepo = prodRepo;
		this.coverageDetailsRepo = coverageDetailsRepo;
		this.policyTermsRepo = policyTermsRepo;
		this.expiryPolicyDetailsRepository = expiryPolicyDetailsRepository;

		this.locationRepository = locationRepository;
		this.clientListRepo = clientListRepo;
		this.employeeRepository = employeeRepository;
		this.irda = irda;
		this.claimsDetailsRepository = claimsDetailsRepository;
	}
	...
	public String createRFQ(CorporateDetailsDto details) {
		...
		cd.setCreateDate(new SimpleDateFormat(pattern).format(new Date()));
...
public String submitRfq(String rfqId, CorporateDetailsDto corporateDetailsDto) {
    ...
    Optional<CorporateDetailsEntity> corporateDetailsOptional = corporateDetailsRepo.findByRfqId(rfqId);
    if (!corporateDetailsOptional.isPresent()) {
        return null;
    }

    CorporateDetailsEntity corporateDetails = corporateDetailsOptional.get();
    updateCorporateDetails(corporateDetails, corporateDetailsDto);

    corporateDetailsRepo.save(corporateDetails);
    return "RFQ Submitted Successfully";
}

private void updateCorporateDetails(CorporateDetailsEntity corporateDetails, CorporateDetailsDto corporateDetailsDto) {
    updateCorporateDetailsFields(corporateDetails, corporateDetailsDto);

    if (corporateDetails.getAppStatus().equals("Closed")) {
        createOrUpdateClientList(corporateDetails);
    } else {
        inactivateClientList(corporateDetails);
    }
}

private void updateCorporateDetailsFields(CorporateDetailsEntity corporateDetails, CorporateDetailsDto corporateDetailsDto) {
...
private void createOrUpdateClientList(CorporateDetailsEntity corporateDetails) {
...
private void inactivateClientList(CorporateDetailsEntity corporateDetails) {
    Optional<ClientList> findByRfqIdOptional = clientListRepo.findByRfqId(corporateDetails.getRfqId());
    if (findByRfqIdOptional.isPresent()) {
        ClientList clientList = findByRfqIdOptional.get();
	...
	public UpdateCorporateDetailsDto updateRFQ(String rfqIdValue, UpdateCorporateDetailsDto corporateDetailsDto) {
		if (rfqIdValue != null && corporateDetailsDto != null) {
			...
			corporateDetailsEntity.setUpdateDate(new SimpleDateFormat(pattern).format(new Date()));
...
public byte[] getEmployeeData(String id) {
    ...
    if (findByRfqId.isPresent()) {
        ...
        throw new RuntimeException("CoverageDetailsEntity not found for rfqId: " + id);
    }
}
	...
	public byte[] getIrdaData(String rfqId) throws IOException {
		...
		List<String> employeeRelations = employeeRepository.findAllRelationShipByRfqId(rfqId).orElse(Collections.emptyList());
	...
	@Override
	public DashBoardDto getCooperateDetailsInfoBasedOnMonthYear(String month, int year) {
    ...
    for (String value : collectStatus) {
        ...
        long count = corporateDetailsRepo.findAll().stream()
               .filter(data1 -> Objects.nonNull(data1.getAppStatus())
                        ...
                        && data1.getLocation()!= null && data1.getLocation().getLocationId().equals(locationId))
               .count();
